...
public class TcpClient extends Thread {
	...
	public void connectToServer(String ipAddress) {
		try {
			clientSocket = new Socket(ipAddress, 8086);
	...
	private void askAndReconnectToServer() {
		if (Controller.getInstance().askForReconnection() == true) {
			closeConnections();
			connectToServer(Controller.getInstance().getIpAddress());
		} else {
			System.exit(0);
		}
	}
	...
	private void receiveBackCommandFromServer() {
		try {
			...
			if (receivedJson != null) {
				...
				Controller.getInstance().executeBackCommand(receivedJson);
			} else {
	...
	public void closeConnections() { // If there are any connections and they are opened, close them in opposite order to opening.
		Controller.getInstance().requestRemovingUserFromList();
		if (outToServer != null) {
			try {
